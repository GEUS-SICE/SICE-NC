...
# output path
opath='/Users/jason/0_dat/MODIS_Greenland_L3_1km_1485_2684_NetCDF/'
...
if JB:
    ...
    coordinates_4326_x=rasterio.open('/Users/jason/0_dat/MODIS_Greenland_L3_1km_1485_2684/lon_1km_1485x2684.tif').read(1)
    coordinates_4326_y=rasterio.open('/Users/jason/0_dat/MODIS_Greenland_L3_1km_1485_2684/lat_1km_1485x2684.tif').read(1)
else:
    #longitude and latitude rasters to convert EPS3413 to EPSG4326
...
for year in range(2003,2020):
    ...
    #load MODIS daily outputs 
    folder_name='H:\\MOD10A1_albedo\\'+str(year)+'\\'
    if JB: folder_name='/Users/jason/0_dat/MODIS_Greenland_L3_1km_1485_2684/'+str(year)+'/'
    year=folder_name.split(os.sep)[-2]
    print(year)
    
    #create nc file
    ofile='C:/Users/Pascal/Desktop/Greenland_1km_albedo_days_60-300_'+str(year)+'.nc'
    if JB: ofile=opath+'Greenland_1km_albedo_days_60-300_'+str(year)+'.nc'
    ...
    root_grp.geospatial_lat_min=latmin ; cc+=1
    root_grp.geospatial_lat_max=latmax ; cc+=1
    root_grp.geospatial_lon_max=lonmax ; cc+=1
    root_grp.geospatial_lon_min=lonmin ; cc+=1
    root_grp.time_coverage_start = str(year)+'-03-01T15:00:00Z' ; cc+=1
    root_grp.time_coverage_end = str(year)+'-10-25T15:00:00Z' ; cc+=1
    ...
    daily_BBAs=sorted(list(glob.glob(folder_name+'*.tif')))
    
    n_files=len(daily_BBAs)
    
    for i,BBA in enumerate(daily_BBAs[0:n_files]):
    # for i,BBA in enumerate(daily_BBAs[0:2]):
        
        print(year,'remaining',n_files-i)
        #read daily albedo
        data_reader=rasterio.open(BBA)
        data=data_reader.read(1)
    
        #store variable name (DOY in the case of MODIS)
        variable_name=BBA.split(os.sep)[-1].split('_')[1].split('.')[0]
        ...
        variable = vars_grp.createVariable(variable_name, 'f4', ('x', 'y'), zlib=True)
