...
# output path
opath='/Users/jason/0_dat/SICE_Greenland_L3_1km_1487_2687_NetCDF/'
...
for year in range(2018,2019):
    ...
    if JB: folder_name='/Users/jason/0_dat/S3/BBA_emp/'
    ...
    for i,date in enumerate(daily_list_that_year):
        if i>=0:
            ymd=daily_list_that_year[i][-10:]
            print(i,ymd,year,'remaining',n_files-i)
            
            #read daily albedo emp
            BBA=daily_list_that_year[i]+'/BBA_emp.tif'
            BBA_emp_data_reader=rasterio.open(BBA)
            BBA_emp_data=BBA_emp_data_reader.read(1)
            ...
            BBA_emp_data[v]=1.
            v=BBA_emp_data<0.565
            BBA_sn_data[v]=BBA_emp_data[v]
            #store variable name 
            variable_name=ymd
            ...
            temp=pd.to_datetime(variable_name,format="%Y-%m-%d")
            date_object=temp.strftime("%j")
            
            variable = vars_grp.createVariable(date_object, 'f4', ('x', 'y'), zlib=True)

            #pass the data    
            BBA_sn_data=np.flipud(BBA_sn_data)
            #pass the data
            variable[:]=BBA_sn_data
            cc+=1
